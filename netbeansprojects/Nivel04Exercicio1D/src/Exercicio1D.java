...
// </editor-fold>
		...
		Passo 1: Leia 15 elementos inteiros de uma matriz A do tipo vetor. 
		
		Passo 2: Construir uma matriz B de mesmo tipo
		Atenção: Todo elemento da matriz B deve ser o quadrado do elemento da matriz A correspondente". 
		
		Passo 3 : Calcular o valor do quadrado dos elementos de A
		...
		*/
		...
		for (int indice = 0; indice < numsA.length; indice++) {
			...
			mensagem1 += numsA[indice] + ", ";
		}
		
		for (int indice = 0; indice < numsB.length; indice++) {
			numsB[indice] = numsA [indice] * 2;
			mensagem2 += numsB[indice] + ", ";
		}
		
		write("Os valores dos elementos de A são: \n" + mensagem1 + 
				"\nOs valores dos elementos de B são: " + mensagem2);
