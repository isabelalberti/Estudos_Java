...
// </editor-fold>
		...
		/*
		Leia duas matrizes A e B de uma dimensão com seis elementos. 
		A matriz A deve aceitar apenas a entrada de valores pares, 
		A matriz B deve aceitar apenas a entrada de valores ímpares. 
		
		Construir uma matriz C que seja o resultado da junção das matrizes A e B, de modo que a matriz C contenha 12 elementos. 
		...
		*/
		...
		for (int i = 0; i < arrayA.length; i++) {
			arrayA[i] = readInteger("Arrray A\n Digite o " + (i + 1) + "º valor par");
						
			if (arrayA[i] %2 !=0) {
				System.out.println("Digite somente valores pares");
				i--;
			}
		...
		for (int i = 0; i < arrayA.length; i++) {
			arrayB[i] = readInteger("Arrray B\n Digite o " + (i + 1) + "º valor impar");
			
			if (arrayB[i] %2==0) {
				System.out.println("Digite somente valores ímpares");
				i--;
			}
		}
