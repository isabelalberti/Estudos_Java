...
// </editor-fold>
		...
		/*
		Passo 1: Ler os valores de quatro notas escolares bimestrais de um aluno
		
		Passo 2: Calcular a média aritmética (variável MD1) desse aluno
		
		Passo 3: Apresentar a mensagem "Aprovado" se a média
obtida for maior ou igual a 7; 
		Caso contrário, 
		Passo 4: Solicitar a quinta nota (nota de exame, representada pela variável NE) 
		
		Passo 5: Calcular uma nova média aritmética (variável MD2) entre a nota
de exame e a primeira média aritmética. 
		
		Passo 6: Se o valor da nova média for maior ou igual a cinco, apresentar a
mensagem "Aprovado em exame"; 
		Caso contrário, 
		Passo 7: Apresentar a mensagem "Reprovado". 
		
		Passo 8: Informar também, o valor da média obtida pelo aluno.
		 */
		Double n1 = readDouble("Qual foi sua nota do 1º Bimestre?");
		Double n2 = readDouble("Qual foi sua nota do 2º Bimestre?");
		Double n3 = readDouble("Qual foi sua nota do 3º Bimestre?");
		Double n4 = readDouble("Qual foi sua nota do 4º Bimestre?");

		Double mD = (n1 + n2 + n3 + n4) / 4.0;
		Double mE = 0.0;

		if (mD >= 7) {
			write(
					"	Você foi Aprovado!!!" + "\n"
					+ "Sua média obtida foi de: " + mD);
		} else {
			mE = readDouble("Qual foi sua nota do exame final?");

			Double mD2 = (mD + mE) / 2.0;

			if (mD2 >= 5) {
				write(
						"Você foi Aprovado!!!" + "\n"
						+ "Sua média obtida foi de: " + mD2);
			} else {
				write(
						"Você foi Reprovado no exame!!!" + "\n"
						+ "Sua média obtida foi de: " + mD2);
			}
		}
