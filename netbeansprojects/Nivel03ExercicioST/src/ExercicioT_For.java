...
// </editor-fold>
		...
		/*
		Uma pessoa quer jogar par ou ímpar contra a máquina.
		
		Passo 1: Considere que a pessoa sempre será par.
		Passo 2: Repita o jogo melhor de três
		Passo 3 : A máquina deverá ser gerado um número aleatório.
		Passo 4: apresente o resultado de quem ganhou.
		
		Lembre-se de que a pessoa sempre irá escolher um número entre 0 e 5 a cada rodada e a maquina também,
		porém o da máquina deverá ser gerado um número aleatório.
		 */
		
		Integer limiteJogadas = 3;
		Integer resto = 0;
		Integer soma = 0;
		
		Integer numJogador = 0;
		Integer numMaquina = 0;
		Integer vitoriaJogador = 0;
		Integer vitoriaMaquina = 0;
		
		for (int cont = 0; cont < limiteJogadas; cont++){
			numJogador = readInteger("Digite um número para jogar Par ou Ímpar");
			numMaquina = (int) (Math.random() * 6);
			soma = numJogador + numMaquina;
			resto = soma % 2;
			
			// é par?
			if (resto == 0) {
				write("Seu número: " + numJogador + 
						"\nNúmero da máquina: " + numMaquina + "\nParabéns!!! Você Ganhou! =P ");
				
				vitoriaJogador ++;
			}
			else {
				write("Seu número: " + numJogador + 
						"\nNúmero da máquina: " + numMaquina + "\nNão foi dessa vez que você ganhou!");
		...
		write("Você ganhou: " + vitoriaJogador + "\nA máquina ganhou: " + vitoriaMaquina);
