...
// </editor-fold>
	...
	Double valor = readWindowDouble("Digite o valor da prestação?");
	Double taxa = readWindowDouble("Qual a taxa cobrada pelo banco pelo atraso?");
	Double tempo = readWindowDouble("Quanto tempo a prestacao esta em atraso?");
	
	Double prestacao = valor + (valor * (taxa/100) * tempo);
	
	writeWindow("O valor total a pagar da prestação é de:" + prestacao + "reais" );
