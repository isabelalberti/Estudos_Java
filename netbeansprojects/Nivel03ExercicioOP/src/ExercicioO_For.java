...
// </editor-fold>
		...
		/*
		Passo 2: Calcular a fatorial desses valores ímpares
		...
		for (int contInicio = 1; contInicio < 10; contInicio++){
			resto = contInicio % 2;
			...
			if (resto != 0) {
				...
				if (contInicio >= 0) {
					for (int contReverso = contInicio; contReverso >= limite; contReverso--)
					 {
						fatorial = fatorial * contReverso;
						// write ("contReverso: " + contReverso);	
					
					}
					write("O resultado da fatorial " + contInicio + " é: " + "\n           " 
							+ fatorial );
