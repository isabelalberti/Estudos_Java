...
// </editor-fold>
		...
		/*
		Leia uma matriz A de uma dimensão com dez elementos numéricos inteiros. 
		Apresentar o total de elementos ímpares existentes na matriz 
		também o percentual do valor total de números ímpares em relação à quantidade total de elementos armazenados na matriz.
		*/
		
		Integer[] arrayA = new Integer [10];
		Integer qtdImpar = 0;
		Integer porcentImpar = 0;
		
		for (int i = 0; i < arrayA.length; i++) {
			arrayA[i]= readInteger("Digite o " + (i + 1) + "º valor para Array A");
			
			if (arrayA[i] % 2 != 0) {
				qtdImpar++;
				porcentImpar = ((100*qtdImpar)/arrayA.length);
			}
		}
		
		System.out.println("Total de valores Impares é de :");
		System.out.println(qtdImpar);
		System.out.println ("O percentual do valor total de números ímpares é de: " + porcentImpar);		
