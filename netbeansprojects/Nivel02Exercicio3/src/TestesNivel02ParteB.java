...
// </editor-fold>
	...
	Integer idade = readWindowInteger("Qual sua idade? ");
...
Caso 2: Idade >0 && <18         ==> Não foi dessa vez
Caso 3: Idade >=18 && <120      ==> O Pai ta on! 
	Caso 3.1: entrada == vip    ==> Chega mais que você é Rei!
	Caso 3.2: entrada != vip    ==> Pelo menos compre uma bebida e divirta-se!
Caso 4: idade >=120             ==> Sai fora matusalém...		
*/
	
		if (idade < 0) {
			mensagem = mensagem + "Tente novamente";
		}
		
		if (idade > 0 && idade < 18) {
			mensagem = mensagem + "Não foi dessa vez";
		}
		
		if (idade > 18 && idade < 120) {
			mensagem += "O Pai ta on! ";
			...
			if (precoPagoNaEntrada >= ENTRADA_VIP) {
				mensagem += "Chega mais que você é Rei! ";
			}
			
			else {
				mensagem += "Pelo menos compre uma bebida e divirta-se!";
			}
		}
		
		if (idade >= 120) {
			mensagem += "Sai fora matusalém...";
		}
