...
// </editor-fold>
		...
		/*
	Passo 1: Leia uma matriz A do tipo vetor com 20 elementos inteiros. 
	
	Passo 2: Construir uma matriz B do mesmo tipo e dimensão da matriz A, 
	
	Passo 3: Cada elemento da matriz B é o somatório de 1 até o valor do elemento correspondente armazenado na matriz A. 
	Se o valor do elemento da matriz A[1] for 5,
	o elemento correspondente da matriz B[1] deve ser 15, 
	pois o somatório do elemento da matriz A é 1+2+3+4+5.
		
	Passo 4: Apresentar os elementos da matriz B. 
		 */
		Integer[] numsA = new Integer[4];
		Integer[] numsB = new Integer[4];
		String mensagem = "";

		for (int indice = 0; indice < numsA.length; indice++) {
			numsA[indice] = readInteger("Digite o " + (indice + 1) + "º para A");
		...
		for (int indice = 0; indice < numsA.length; indice++) {
			...
			for (int cont = 1; cont <= numsA[indice]; cont++) {
				soma += cont;
			...
			mensagem += numsB[indice] + ",";

		}

		write("Os números de B são " + mensagem);
